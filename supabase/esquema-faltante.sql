-- Parte que faltó: seguridad, funciones y realtime.
--
-- El primer intento de correr `esquema.sql` se cortó después de los índices, así
-- que las tablas quedaron creadas pero SIN RLS y sin funciones. Esto completa lo
-- que falta. Es exactamente el tramo final de `esquema.sql` —esa sigue siendo la
-- fuente de verdad—, separado solo para que el pegado sea corto y no se trunque.
--
